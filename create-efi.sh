...
echo
echo
echo efibootmgr --disk /dev/"$boot_disk" --part "$boot_partition" --create --label "$label" --loader "$rel_path2"/vmlinuz-"$kernel_name" --unicode "$unicode_arg" --verbose
echo efibootmgr --disk /dev/"$boot_disk" --part "$boot_partition" --create --label "$label-fallback" --loader "$rel_path2"/vmlinuz-"$kernel_name" --unicode "$unicode_arg_fallback" --verbose
echo
echo
efibootmgr --disk /dev/"$boot_disk" --part "$boot_partition" --create --label "$label" --loader "$rel_path2"/vmlinuz-"$kernel_name" --unicode "$unicode_arg" --verbose
efibootmgr --disk /dev/"$boot_disk" --part "$boot_partition" --create --label "$label-fallback" --loader "$rel_path2"/vmlinuz-"$kernel_name" --unicode "$unicode_arg_fallback" --verbose
